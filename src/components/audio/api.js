...
Visualizer.prototype = {
    ...
    _prepareAPI: function() {
        ...
        try {
            ...
            this._updateInfo('!您的浏览器不支持AudioContext', false);
            console.log(e);
        }
    },
    // 事件监听
    _addEventListner: function() {
        ...
        dropContainer.addEventListener("dragenter", function() {
            document.getElementById('fileWrapper').style.opacity = 1;
            that._updateInfo('正在拖拽到页面上', true);
        }, false);
    ...
    _start: function() {
        ...
        fr.onload = function(e) {
            ...
            that._updateInfo('正在解析音频', true);
            audioContext.decodeAudioData(fileResult, function(buffer) {
                that._updateInfo('Decode succussfully,start the visualizer', true);
                that._visualize(audioContext, buffer);
            }, function(e) {
                that._updateInfo('!文件解析失败', false);
                console.error(e);
            });
        };
        fr.onerror = function(e) {
            that._updateInfo('!文件读取失败', false);
            console.error(e);
        };
    ...
    _visualize: function(audioContext, buffer) {
        ...
        this._updateInfo('开始播放 ' + this.fileName, false);
        this.info = '开始播放 ' + this.fileName;
        document.getElementById('fileWrapper').style.opacity = 0.2;
